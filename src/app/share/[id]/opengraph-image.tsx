...
export default async function Image({ params }: Props) {
  ...
  if (!result.ok) {
    ...
    console.log(`[og-image] PROFILE_NOT_FOUND id=${id} error=${result.error}`);
  ...
  console.log(`[og-image] PROFILE_FOUND id=${id}`);
  ...
  const photoSrc = await resolvePhotoForSatori(profilePhoto, '[og-image]');

  console.log(`[og-image] IMAGE_RESPONSE_START id=${id}`);
  ...
  console.log(`[og-image] IMAGE_RESPONSE_SUCCESS id=${id}`);
  return response;
}
